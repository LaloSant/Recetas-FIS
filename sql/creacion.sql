CREATE USER RECETARIO IDENTIFIED BY FIS;
GRANT ALL PRIVILEGES TO RECETARIO;
...
CREATE TABLE ACCIONUSUARIO (
	ID_ACCIONUSUARIO NUMBER GENERATED BY DEFAULT AS IDENTITY
		CONSTRAINT USRACC_ID_ACCUSR_PK PRIMARY KEY
	,ID_USUARIO       NUMBER
		CONSTRAINT USRACC_ID_USR_NN NOT NULL
		CONSTRAINT USRACC_ID_USR_FK
			REFERENCES USUARIOS ( ID_USUARIO )
	,ID_RECETA        NUMBER
		CONSTRAINT USRACC_ID_RECT_NN NOT NULL
		CONSTRAINT USRACC_ID_RECT_FK
			REFERENCES RECETAS ( ID_RECETA )
	,INTERACCION      VARCHAR2(3)
		CONSTRAINT USRACC_INT_NN NOT NULL
		CONSTRAINT USRACC_INT_CK CHECK ( INTERACCION IN ( 'FAV'
		                                                 ,'CAL' ) )
	,DETALLE          NUMBER(1)	-- NULO O DE 0 A 5 EN CASO DE CALIFICAR
);
...
CREATE TABLE PATROCINADOR(
	ID_PATROCINADOR     NUMBER GENERATED BY DEFAULT AS IDENTITY
		CONSTRAINT PATROCINADOR_ID_PK PRIMARY KEY
	,NOMBRE VARCHAR2(100) 
		CONSTRAINT PATROCINADOR_NOMBRE_NN NOT NULL
	,ENLACE VARCHAR2(100) 
);

ALTER TABLE INGREDIENTES ADD ID_PATROCINADOR NUMBER REFERENCES PATROCINADOR(ID_PATROCINADOR);